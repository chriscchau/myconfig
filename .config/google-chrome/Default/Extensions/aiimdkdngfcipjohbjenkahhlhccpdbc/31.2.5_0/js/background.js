...
window.addEventListener( "load", function(){
	...
	chrome.runtime.setUninstallURL("https://chrome.google.com/webstore/detail/flash-video-downloader/aiimdkdngfcipjohbjenkahhlhccpdbc");
...
function gampSet(){
	...
	if( now - fvdSingleDownloader.Prefs.get( "fvd.gamp.check" ) > INTERVAL_TO_DISPLAY )		{
...
function gampBackend(data){
    ...
    fvdSingleDownloader.Utils.postAJAX( "https://www.google-analytics.com/collect",
    									param,
    									function(e){	   });
...
function createCidGA(){
    var date = new Date();
    var cid = "ch-" + date.getFullYear() + "-" + (date.getMonth()+1) + "-" + date.getDate() + "-" + Math.ceil(Math.random() * 1e15)
